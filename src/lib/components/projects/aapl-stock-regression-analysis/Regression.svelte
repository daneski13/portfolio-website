<script>
	...
	const raw = `<div class="input input-md"><span class="heading"><h1 id="appleregressionanalysis">Apple Regression Analysis<a href="#appleregressionanalysis"> ¶</a></h1></span>
...
<p>The Scenario:</p>
<p>As a financial analyst for theoretical smartphone manufacturer I am tasked with providing an analysis of the smartphone industry's market risk to the company's CEO. This theoretical company wants to use behavior of Apple Inc.'s (AAPL) stock, a leading firm in the industry and its main competitor, as a proxy for the industry as a whole. The company is considering a new smartphone manufacturing and launch project, but is concerned about the significant investment required.</p>
...
data<span class="token punctuation">.</span>describe<span class="token punctuation">(</span><span class="token punctuation">)</span></code></pre></div><pre class="output output-stream">[*********************100%***********************]  3 of 3 completed</pre><div class="output output-html-table">
...
</div><div class="input input-md"><span class="heading"><h3 id="convertpricestoriskadjustedrates">Convert prices to risk-adjusted rates<a href="#convertpricestoriskadjustedrates"> ¶</a></h3></span></div><div class="input input-code"><pre class="language-python"><code class="language-python"><span class="token comment"># Convert the prices of the S&amp;P and Apple to their percentage change from the previous week (weekly rate of return)</span>
...
data<span class="token punctuation">.</span>head<span class="token punctuation">(</span><span class="token punctuation">)</span></code></pre></div><pre class="output output-stream">Prices and the risk-free rate converted to their weekly rates of return:</pre><div class="output output-html-table">
...
</div><pre class="output output-stream">Returns adjusted by risk-free rate:</pre><div class="output output-html-table">
...
</div><div class="input input-md"><span class="heading"><h2 id="plotsandregressionanalysis">Plots and Regression Analysis<a href="#plotsandregressionanalysis"> ¶</a></h2></span></div><div class="input input-code"><pre class="language-python"><code class="language-python"><span class="token keyword">import</span> statsmodels<span class="token punctuation">.</span>api <span class="token keyword">as</span> sm
...
DASH_COLOR <span class="token operator">=</span> <span class="token string">"dimgrey"</span></code></pre></div><div class="input input-md"><span class="heading"><h3 id="regressionmodel">Regression Model<a href="#regressionmodel"> ¶</a></h3></span></div><div class="input input-code"><pre class="language-python"><code class="language-python"><span class="token comment"># S&amp;P 500 is the independent variable and Apple is the dependent variable</span>
...
plt<span class="token punctuation">.</span>show<span class="token punctuation">(</span><span class="token punctuation">)</span></code></pre></div><pre class="output output-stream">/var/folders/d6/rpd6b_591txdmpf_4f2f21740000gn/T/ipykernel_20068/3378242753.py:30: UserWarning: FixedFormatter should only be used together with FixedLocator
...
plot.set_yticklabels(['{:.0f}%'.format(y) for y in plot.get_yticks()])</pre><img src="${img_path_prefix}Output_10_1.webp" alt="${img_alt_text[0]}"><div class="input input-md"><span class="heading"><h3 id="residualplotsandmodelstatistics">Residual Plots and Model Statistics<a href="#residualplotsandmodelstatistics"> ¶</a></h3></span></div><div class="input input-code"><pre class="language-python"><code class="language-python">res_plots<span class="token punctuation">,</span> axs <span class="token operator">=</span> plt<span class="token punctuation">.</span>subplots<span class="token punctuation">(</span>nrows<span class="token operator">=</span><span class="token number">3</span><span class="token punctuation">,</span> ncols<span class="token operator">=</span><span class="token number">1</span><span class="token punctuation">,</span> figsize<span class="token operator">=</span><span class="token punctuation">(</span><span class="token number">8</span><span class="token punctuation">,</span> <span class="token number">12</span><span class="token punctuation">)</span><span class="token punctuation">)</span>
...
<span class="token keyword">print</span><span class="token punctuation">(</span>model<span class="token punctuation">.</span>summary<span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span></code></pre></div><img src="${img_path_prefix}Output_12_0.webp" alt="${img_alt_text[1]}"><pre class="output output-stream">OLS Regression Results                            
==============================================================================
...
<ol>
<li>Linear relationship<ul>
<li>A correlation coefficient of 0.74 and the SCL plot suggest a strong linear relationship between the returns of the market and the returns of Apple.</li></ul></li>
<li>Independence of residuals<ul>
<li>The residual time series plot shows no evidence of autocorrelation.</li>
<li>The Durbin-Watson statistic is 1.927 (very close to 2), indicating that the residuals are independent.</li></ul></li>
<li>Homoscedasticity (the residuals have constant variance at every level of x)<ul>
<li>The fitted values vs residual plot reflects a pretty constant variance of residuals around 0 for any given value of the market's return, nearly all the residuals are within +/- 5 of 0 regardless of the predicted value.</li></ul></li>
<li>Normality (the residuals are normally distributed)<ul>
<li>An approximate normal distribution of residuals is evident from the distribution of residuals plot</li></ul></li>
</ol>
<span class="heading"><h3 id="marketrisk">Market Risk<a href="#marketrisk"> ¶</a></h3></span>
...
<span class="heading"><h4 id="marketvolatilitybeta">Market Volatility - Beta<a href="#marketvolatilitybeta"> ¶</a></h4></span>
...
<span class="heading"><h4 id="riskadjustedreturnalpha">Risk Adjusted Return - Alpha<a href="#riskadjustedreturnalpha"> ¶</a></h4></span>
...
<p>However, this alpha is relatively small, so Apple's return could be sufficiently explained by beta alone which means that Apple's returns (and the return of the smartphone industry) is very dependent on the performance of the broader market.</p>
...
<p>If the economy is anticipated to be favorable, the firm should definitely proceed with the project. If the economy is predicted to stagnate or have a slight downturn the firm should weigh the risks and rewards of the project to decide whether or not to proceed.</p></div>`;
</script>
...
<svelte:head>
	...
	<script
		...
		crossorigin="anonymous"
	></script>
	<script defer src="https://cdn.plot.ly/plotly-2.20.0.min.js" crossogin="anonymous"></script>
</svelte:head>

<div bind:this={container} class="jupyter">
	{@html raw}
</div>
...
<style>
	...
	/* Link on hover */
	.jupyter :global(.heading > * > a) {
		visibility: hidden;
	}
	.jupyter :global(.heading:hover > * > a) {
		visibility: visible;
	}
	div :global(.heading > * > a:hover) {
		color: rgb(69, 69, 255);
	}
	...
	/* Controls all inputs */
	.jupyter :global(.input) {
		max-width: 100%;
	}

	/* Controls the code blocks */
	.jupyter :global(.input-code) {
		margin-bottom: 1.5em;
		font-size: 0.95em;
	}

	/* Controls the markdown blocks */
	.jupyter :global(.input-md) {
		margin-top: 1.5em;
	}

	/* Controls all output blocks */
	.jupyter :global(.output) {
		display: block;
		max-width: 100%;
		overflow: auto;
	}
	...
	.jupyter :global(.output-html-table) {
		font-size: 0.85em;
	}
	/* Table formatting */
	.jupyter :global(th) {
	...
	/* Table headings */
	.jupyter :global(thead > tr > th) {
		background-color: #ccc;
		border: 0px;
	}
	.jupyter :global(thead > tr:nth-child(2) > th) {
		background-color: #ddd;
	}
	/* Highlight Every other row */
	.jupyter :global(tr:nth-child(even)) {
		background-color: #ddd;
	}
	/* Give rows a border */
	.jupyter :global(tr) {
